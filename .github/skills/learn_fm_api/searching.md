...
Several list endpoints in the Security Cloud Control Firewall Manager API allow you to search across multiple fields using the Lucene query syntax.
...
- All dates must comply with [RFC 3339](https://tools.ietf.org/html/rfc3339).
- To make an exclusive time range query (for the lower bound, upper bound, or both), replace the `[` with a `{` or the `]` with a `}`:
...
- Use the `*` wildcard to make open-ended searches:
...
- When making these API calls using cURL or Postman, you must URL-encode the query. For example:
  - `/foo/bar?q=loginTime:[2024-12-17Z10:00:00Z TO 2024-12-17Z13:00:00Z]` should be encoded as  
  `/foo/bar?q=loginTime:%5B2024-12-17Z10:00:00Z%20TO%202024-12-17Z13:00:00Z%5D`.
