...
The following sections introduce you to the resources available in the Security Cloud Control Firewall Manager API and provide instructions for your first API request. Sample requests are shown using curl, a CLI tool.

## Supported RESTful Operations
The resources in the Security Cloud Control API support one or more GET, POST, PATCH, PUT, and DELETE operations.

> **Note:** Not every resource supports all tThese operations.
...
- **POST**: TThese operations are used to create resources or to trigger asynchronous operations. All POST operations that use an action verb initiate asynchronous operations, which can be tracked using the Security Cloud Control transaction API.
- **PATCH**: These operations are used to modify existing resources.
- **PUT**: TThese operations are used by the cloud-delivered Firewall Management Center (cdFMC) API endpoints to modify resources.
- **DELETE**: These operations are used to remove resources. All DELETE operations in the Security Cloud Control API are idempotent.
...
To use the Security Cloud Control API, you must have an API token. We recommend creating an API-only user in your Security Cloud Control tenant, and generating a token for it.
...
Base URL
Cisco Security Cloud Control is deployed across multiple regions, and the API is available in all of them. The server URLs for each region is as follows:
...
> **Note:** If you are still using the old base URLs (https://edge.<region>.cdo.cisco.com), they will continue to work. But please switch your scripts to use the new URLs.
...
Use this curl command to make a request to retrieve the list of devices from the `/v1/inventory/devices` endpoint:
...
> **Note:** Replace `us` with the region you are using.

#### Response

The response returned is a JSON object that has some metadata on the request itself, including:
...
> **Note:** The `limit` and `offset` values can be set as query parameters to the request, and can be used to page through a list. By default, `limit` is set to 50, and `offset` to 0.

### 3. Get the UID and Domain UUID of the cdFMC

#### Request

Use this curl command to request the retrieval of the UUID of the cloud-delivered FMC (cdFMC) on your tenant from the `/v1/inventory/managers` endpoint with the query parameter `q` set to `deviceType:CDFMC`:
...
If your tenant has a cdFMC, the response will be a list of size 1. If not, the response will be an empty list. A non-empty response should look something like this:
...
The response field items[0].uid contains the unique identifier of the cdFMC on your tenant. This is a UUIDv4. If you are using jq to parse the response, extract the UID with this command:

```bash
jq -r '.items[0].uid'
Get the domain UUID from the response
The response field items[0].fmcDomainUid contains the unique identifier of the cdFMC on your tenant. This is a UUIDv4. If using jq to parse the response, you can extract the domain UUID from the fmcDomainUid field with this command:

```bash
jq -r '.items[0].fmcDomainUid'
4. Get all access policies on a cdFMC
> **Note:** All the cdFMC API endpoints are prefixed by /v1/cdfmc. These endpoints are compatible with the endpoints used to communicate with an on-premise FMC. So, for example, if you make an API call to the FMC to fetch all access policies using the endpoint
...
Request
Use this curl command to request the retrieval of the UUID of the cdFMC on your tenant from the /v1/cdfmc/api/fmc_config/v1/domain/<domainUid>/policy/accesspolicies endpoint.
...
Responses
The response will be a JSON object that is similar to this example. Note that the response will vary depending on the access policies configured in your cdFMC.
...
To retrieve the details of an access policy in cdFMC, you need the id (represented as a universally unique UUIDv4 identifier) of the access policy, which you can retrieve using the API call described in the previous example.

Request
Use this curl command to make a request to retrieve the UUID of the Cloud-delivered FMC on your tenant from the /v1/cdfmc/api/fmc_config/v1/domain/<domainUid>/policy/accesspolicies/<id> endpoint.
...
Replace e276abec-e0f2-11e3-8169-6d9ed49b625f with the domain UUID you retrieved in the previous step.
Replace 0248CDE2-456F-0ed3-0000-004294967323 with the id of the access policy you want to retrieve.
Replace us with your region
Response
The response will be a JSON object that is similar to this example. Note that the response will vary depending on the access policy you selected.
